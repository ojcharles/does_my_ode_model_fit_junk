...
# each function generates a text string, that can be consumed by 
# the initial parameter estimates are tuned for cmv untreated data with viral load in log10 space. Normalised to 0-1.
...
MwTCLESIC1 = function(fix_dS = F, fix_V_v = F, fix_dI = F, fix_dE = F, fix_gamma = F){
  m = "function() {
    ini({
      add.err = 1
      tbeta = 4
  ...
  if(fix_V_v){
    m = gsub("tV_v = -6","",m)
    m = gsub("eta.V_v ~ 1","",m)
    m = gsub("E_v(0) = exp(tV_v + eta.V_v)","V_v(0) = 0",m,fixed = T)
  }
  if(fix_gamma){
    m = gsub("tgamma = -6","",m)
...
#suppressWarnings(nlmixr(object = eval(parse( text = paste(text = MwTCLESIC1(F,T,F,F,F)))), vl_df, list(print=0), est = "saem"))
...
#suppressWarnings(nlmixr(object = eval(parse( text = paste(text = MwTCLEDIS2(F,F,F,F,F)))), vl_df, list(print=0), est = "saem"))
...
#suppressWarnings(nlmixr(object = eval(parse( text = paste(text = SMEIMCs9(F)))), vl_df, list(print=0), est = "saem"))
